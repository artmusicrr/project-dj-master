...
const deleteImageApi = async (payload: DeleteImagePayload): Promise<string> => {
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/gallery/image/${payload.filename}`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${user?.token}`,
...
function* uploadImageSaga(
  payload: UploadImagePayload,
): Generator<Effect, void, GalleryTypes> {
  try {
...
function* deleteImageSaga(
  action: ReturnType<typeof deleteImageRequest>,
...
function* watchGallerySagas(): Generator<Effect, void, any> {
  while (true) {
    ...
    if (action.type === GalleryActionTypes.FETCH_GALLERY_REQUEST) {
      ...
      const deleteTask = () =>
        deleteImageSaga(action as ReturnType<typeof deleteImageRequest>)
